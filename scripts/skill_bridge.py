...
async def _fts_chunk_ids(brain_ids: list[str], query: str, limit: int) -> set[str]:
    ...
    async with get_session() as session:
        ...
        except Exception as exc:  # malformed FTS query — degrade to dense-only
            _log(f"[context] FTS skipped: {exc}")
            return set()
...
async def cmd_context(brain_ids: list[str], query: str, k: int, all_brains: bool = False) -> None:
    ...
    client = GeminiClient()
    try:
        _log(f"[context] embedding query across {len(brain_ids)} brain(s) (1 embed call, zero generate)")
    ...
    inclause = ",".join(f":b{i}" for i in range(len(brain_ids)))
    sql = text(
        "SELECT c.id, c.brain_id AS bid, c.start_time, c.text, v.video_id AS yt, v.title, "
        "       v.channel_name AS creator, b.name AS brain_name, e.embedding "
        "FROM chunks c "
    ...
    if not rows:
        _emit({"error": "no_chunks_for_brains", "brain_ids": brain_ids})
        return
    ...
    for r in rows:
        ...
        if r.id in fts_hits:
            sim += 0.05  # small lexical boost for keyword matches
        scored.append((sim, r))
    ...
    if n > 1:
        ...
        for sim, r in scored:
            if counts.get(r.bid, 0) < per:
                ...
                seen.add(id(r))
                top.append((sim, r))
        for sim, r in scored:  # backfill remaining slots with the global best
            if len(top) >= k:
                break
            if id(r) not in seen:
                seen.add(id(r))
            ...
            "results": [
                {
                    ...
                    "score": round(sim, 4),
                    "lexical_match": r.id in fts_hits,
                    "text": r.text,
                }
                for sim, r in top
            ],
...
async def cmd_save(path: str) -> None:
    ...
    if models:
        client = GeminiClient()
        try:
            _log(f"[save] embedding {len(models)} claims (zero generate)")
            vecs = await client.embed_texts([m.claim for m in models])
            for m, vec in zip(models, vecs):
                await store_observation_embedding(m.id, client.embed_model, client.embed_dims, vec)
                embedded += 1
        finally:
            await client.close()
    ...
    _emit({
        ...
        "note": "Zero generate. Observations persisted with citations + embeddings; "
                "re-read with `observations` or build the consensus report.",
    })
...
async def cmd_report(brain_ids: list[str], all_brains: bool, out_path: str | None) -> None:
    ...
    (mirrors the project's trust rule). Single brain reuses the persisted
    per-brain clusters (reclustering to pick up any new observations). Multiple
    brains cluster IN MEMORY so a theme can span creators without disturbing the
    stored per-brain clusters. Zero generate (embeddings only).
    """
    await init_database()
    ...
    if not cross:
        ...
        client = GeminiClient()
        try:
            n_clusters = await recluster(bid, client)
        finally:
            await client.close()
        ...
        sources = intel["founders"]
        clusters = None  # cross-creator clusters live in the shared module, not surfaced
        top = [
